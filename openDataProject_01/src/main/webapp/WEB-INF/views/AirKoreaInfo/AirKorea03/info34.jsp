					...
					<div class="info_main_div03">
						...
						<div class="info_main_div04" align="center" style="width: 500px; font-size: small;">
							<div>
								...
								범례 <img alt='good' src='./resources/assets/img/stat_01.png'> 좋음(0~2) <img alt='good' src='./resources/assets/img/stat_02.png'> 보통(2.01~9) <img alt='good' src='./resources/assets/img/stat_03.png'> 나쁨(9.01~15) <img alt='good' src='./resources/assets/img/stat_04.png'> 매우나쁨(15.01~)
							</div>
						</div>
					</div>
		...
		$(function() {
			...
			$.ajax({
				...
				success : function(result) {
					...
					function calculateAverageCO(sidoObject) { // 각 지역별 오존 평균 구하는 함수
						
						  var totalCOValue = 0; // 전체 오존 값
						  var count = 0;

						  for (var i = 0; i < sidoObject.length; i++) {
						    var COValue = Number(sidoObject[i].coValue); // 각 지역의 [i]번째 오존 값
						    
						    if (isNaN(COValue)) { // 통신장애일 경우 '-' 값을 '0'으로 치환
						    	  COValue = 0;
					    	}

						    if (COValue !== '-') { // 통신장애일 경우 '-' 값을 '0'으로 치환
						      totalCOValue += COValue; 
						      count++;
						    }
						  }
						  
						  return count > 0 ? (totalCOValue / count).toFixed(3) : 0; // 통신장애일 경우 값을 0 반환 아닐 경우 오존값 반환
						}
						...
						sidoName.forEach(function(name) {
						  var COval = Number(calculateAverageCO(sidoObject[name])); // 각 지역별 오존 평균 값
						  range.push(COval);
						  
							if ( 0 <= COval && COval <=2) {
								gradeImg = good;
							} else if (2 < COval && COval <=9) {
								gradeImg = normal;
							} else if (9 < COval && COval <=15) {
								gradeImg = bad;
							} else if (15 < COval ) {
								gradeImg = verybad;
							} else {
