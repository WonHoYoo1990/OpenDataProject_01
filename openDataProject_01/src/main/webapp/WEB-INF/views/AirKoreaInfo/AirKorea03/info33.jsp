					...
					<div class="info_main_div03">
						...
						<div class="info_main_div04" align="center" style="width: 600px; font-size: small;">
							<div>
								...
								범례 <img alt='good' src='./resources/assets/img/stat_01.png'> 좋음(0~0.03) <img alt='good' src='./resources/assets/img/stat_02.png'> 보통(0.031~0.06) <img alt='good' src='./resources/assets/img/stat_03.png'> 나쁨(0.061~0.2) <img alt='good' src='./resources/assets/img/stat_04.png'> 매우나쁨(0.201~)
							</div>
						</div>
					</div>
		...
		$(function() {
			...
			$.ajax({
				...
				success : function(result) {
					...
					function calculateAverageNO2(sidoObject) { // 각 지역별 오존 평균 구하는 함수
						
						  var totalNO2Value = 0; // 전체 오존 값
						  var count = 0;

						  for (var i = 0; i < sidoObject.length; i++) {
						    var NO2Value = Number(sidoObject[i].no2Value); // 각 지역의 [i]번째 오존 값
						    
						    if (isNaN(NO2Value)) { // 통신장애일 경우 '-' 값을 '0'으로 치환
						    	  NO2Value = 0;
					    	}

						    if (NO2Value !== '-') { // 통신장애일 경우 '-' 값을 '0'으로 치환
						      totalNO2Value += NO2Value; 
						      count++;
						    }
						  }
						  
						  return count > 0 ? (totalNO2Value / count).toFixed(3) : 0; // 통신장애일 경우 값을 0 반환 아닐 경우 오존값 반환
						}
						...
						sidoName.forEach(function(name) {
						  var NO2val = Number(calculateAverageNO2(sidoObject[name])); // 각 지역별 오존 평균 값
						  range.push(NO2val);
						  
							if ( 0 <= NO2val && NO2val <=0.03) {
								gradeImg = good;
							} else if (0.03 < NO2val && NO2val <=0.06) {
								gradeImg = normal;
							} else if (0.06 < NO2val && NO2val <=0.2) {
								gradeImg = bad;
							} else if (0.2 < NO2val ) {
								gradeImg = verybad;
							} else {
